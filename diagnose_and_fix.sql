-- Полная диагностика и исправление пользователя admin
...
-- ШАГ 2: Удаление всех вариантов admin (на случай пробелов или других символов)
SELECT '=== УДАЛЕНИЕ СТАРЫХ ЗАПИСЕЙ ===' AS info;
-- Используем id для безопасного удаления
DELETE FROM users WHERE id IN (SELECT id FROM (SELECT id FROM users WHERE username = 'admin') AS temp);
DELETE FROM users WHERE id IN (SELECT id FROM (SELECT id FROM users WHERE TRIM(username) = 'admin') AS temp);
DELETE FROM users WHERE id IN (SELECT id FROM (SELECT id FROM users WHERE LOWER(username) = 'admin') AS temp);
